    ...
    NPCType(Class<?> entityClass, String newName, double holoHeight, String... methods) {
        ...
        try {
            if (Utils.versionNewer(14)) {
                this.nmsEntityType = ((Optional)((Method)CacheRegistry.ENTITY_TYPES_A_METHOD.load()).invoke(null, new Object[] { this.bukkitEntityType.getKey().getKey().toLowerCase() })).get();
                this.constructor = entityClass.getConstructor(new Class[] { CacheRegistry.ENTITY_TYPES_CLASS, CacheRegistry.WORLD_CLASS });
            } else {
                this.constructor = entityClass.getConstructor(new Class[] { CacheRegistry.WORLD_CLASS });
            }
