...
public enum NPCType {
    ...
    private NPCType(Class<?> entityClass, String newName, double holoHeight, String... methods) {
        this.holoHeight = holoHeight;
        this.customizationLoader = entityClass == null ? null : new CustomizationLoader(this.bukkitEntityType = EntityType.valueOf(newName.length() > 0 ? newName : this.name()), Arrays.asList(methods));
        if (entityClass != null && !entityClass.isAssignableFrom(CacheRegistry.ENTITY_PLAYER_CLASS)) {
            try {
                if (Utils.versionNewer(14)) {
                    this.nmsEntityType = ((Optional)((Method)CacheRegistry.ENTITY_TYPES_A_METHOD.load()).invoke((Object)null, this.bukkitEntityType.getKey().getKey().toLowerCase())).get();
    ...
    private NPCType(Class<?> entityClass, double holoHeight, String... customization) {
    ...
    public void updateCustomization(NPC npc, String name, String[] values) {
        if (this.customizationLoader.contains(name)) {
            try {
                Method method = (Method)this.customizationLoader.getMethods().get(name);
                ...
                throw new IllegalStateException("can't invoke method: " + name, var5);
            }
        }
    }
}
